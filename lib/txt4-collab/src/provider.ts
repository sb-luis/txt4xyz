...
// RFC 6455 private-use close code range (4000-4999): any code here from the relay is terminal, never reconnect on it.
const TERMINAL_CLOSE_CODE_MIN = 4000;
const TERMINAL_CLOSE_CODE_MAX = 4999;

// Counterpart in apps/backend-go/internal/relay/relay.go; the range check above stays authoritative for unnamed future codes.
...
export class SyncProvider {
  ...
  constructor(options: SyncProviderOptions) {
    ...
    // origin === this means the relay already fanned this out; re-broadcast only for a newcomer in `added`, which terminates after one reply.
    ...
    // Guarded so the integration test can drive the provider from Node, without a browser, against a real relay.
  ...
  private connect(): void {
    ...
    socket.onclose = (event) => {
      ...
      // Only a connection that survived counts as success, or a relay rejecting the room id resets backoff every attempt into a hot loop.
  ...
  // Written raw, ahead of the send chain: the room id is a routing header the relay must read, not payload it can decrypt.
