...
describe("the encode/decode seam", () => {
  ...
  it("routes every outbound payload through encode and every inbound payload through decode", async () => {
    ...
    // Mimics a real relay: consumes each peer's first frame (the room id), forwards only sync traffic after it.
    ...
    // Convergence above only happens if every sent frame passed through encode()/decode().
...
describe("destroy", () => {
  ...
  it("closes the socket, stops listening for doc updates, and cancels any pending reconnect", async () => {
    ...
    const sockets: FakeSocket[] = [];
    const doc = new Y.Doc();
    // Destroyed explicitly (as useRoom does) so its outdated-state check interval doesn't skew this test's timer count.
...
describe("message envelope", () => {
  ...
  it("ignores an unknown message type instead of throwing", async () => {
    ...
    // handleMessage runs detached (`void this.handleMessage(...)`), so a throw surfaces as an unhandled rejection, not a sync exception.
...
describe("awareness announce-to-newcomer", () => {
  // Forwards every post-handshake frame to every other current member, mirroring real room fan-out.
  ...
  it("makes an already-connected peer announce itself to a newcomer, and terminates instead of echoing forever", async () => {
    ...
    // A is alone here; regression check for a bug where a joiner's participant list showed only themselves until a peer moved a cursor.
