...
describe("langJsRunner", () => {
  it("produces one step per executed line", async () => {
    const { steps, error } = await langJsRunner.run('console.log("a")\nconsole.log(1 + 1)');
  ...
  it("joins multiple console.log arguments with a space, like the real console", async () => {
    const { steps, error } = await langJsRunner.run('console.log("a", 1, "b")');
    expect(error).toBeNull();
